...
func TestTurnCostExperiment(t *testing.T) {
	costStr := os.Getenv("TURN_COST")
	if costStr == "" {
		t.Skip("set TURN_COST")
	}
	cost, _ := strconv.ParseFloat(costStr, 64)
	data, _ := os.ReadFile(filepath.Join("..", "settings", "default.json"))
	var g config.Globals
	json.Unmarshal(data, &g)
	g.HealthChangeFromTurning = cost
	config.SetGlobals(&g)

	var seeds []int
	for _, f := range strings.Split(os.Getenv("TURN_SEEDS"), ",") {
		n, _ := strconv.Atoi(f)
	...
	for _, seed := range seeds {
		...
		type tally struct{ orgs, ph, moves, turns, spawns, dist, age, moveScore float64 }
		var per []tally
		cycles := 0
		for cycles < capCycles && !sim.IsDone() {
			...
			for _, o := range sim.organismManager.Organisms() {
				...
				case organism.StatusMoveSuccess:
					tl.moves++
				case organism.StatusTurnLeft, organism.StatusTurnRight:
					tl.turns++
		...
		n := float64(len(half))
		t.Logf("RESULT turn=%-7s seed=%-5d %-13s pop=%7.1f avgPh=%5.2f move%%=%6.2f turn%%=%6.2f spawn%%=%5.2f movesPerSpawn=%6.2f distPer100Age=%6.2f moveScore=%5.1f",
			costStr, seed, end, h.orgs/n, h.ph/n, 100*div(h.moves, h.orgs), 100*div(h.turns, h.orgs), 100*div(h.spawns, h.orgs),
