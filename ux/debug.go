...
func (d *Debug) render() *ebiten.Image {
	...
	info = fmt.Sprintf("%s\n  ActionUpdate: %10s", info, d.simulation.OrganismUpdateTime)
	info = fmt.Sprintf("%s\n  ActionResolve:%10s", info, d.simulation.OrganismResolveTime)
	info = fmt.Sprintf("%s\n OrganismUpdate:%10s", info, d.simulation.OrganismUpdateTime+d.simulation.OrganismResolveTime)
