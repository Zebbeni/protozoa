...
func (t Traits) copyMutated() Traits {
	...
	// phEffect = previous +- 0.0001, bounded by MaxOrganismPhGrowthEffect (and -1 * MaxOrganismPhGrowthEffect)
	phEffect := mutateFloat(t.PhGrowthEffect, .0001, c.MaxOrganismPhGrowthEffect()*-1, c.MaxOrganismPhGrowthEffect())
