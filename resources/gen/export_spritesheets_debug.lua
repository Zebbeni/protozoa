...
--
-- Diagnostic prints around each teardown step: if Aseprite freezes
-- between the export loop and the summary alert, the last marker
-- printed tells us which call hung. Remove these once we've
-- identified and fixed the culprit.
-- ------------------------------------------------------------------

print("[teardown] export loop done; closing scratch sprite")
copy:close()
print("[teardown] scratch closed; removing temp file")
os.remove(tempPath)
print("[teardown] temp removed; switching active sprite back")
if origSprite then
    app.activeSprite = origSprite
end
print("[teardown] active sprite restored; building summary lines")
...
print("[teardown] summary built; about to show alert")
app.alert{ title = "Export spritesheets", text = lines }
print("[teardown] alert dismissed; script done")
